...
3. Você verá três elementos associados a lh_FAIAD – Lakehouse, Modelo semântico e Ponto de extremidade de SQL. Exploramos o Lakehouse e criamos consultas de visual usando o ponto de extremidade da análise SQL em um laboratório anterior. Selecione **FAIAD_<nome de usuário>** na navegação esquerda e escolha o **Ponto de extremidade de análise de SQL lh_FAIAD** para continuar a explorar essa opção. Você será direcionado à **exibição de SQL** do explorador.
    ...
    Se desejar explorar os dados antes de criar um modelo de dados, você poderá usar SQL para fazer isso. A primeira opção é a consulta visual, que usamos no laboratório anterior junto com a Opção 2, escrevendo o código TSQL. A segunda opção é escrever código T-SQL. É uma opção conveniente para desenvolvedores. Vamos explorar isso.
...
8. No painel **esquerdo Explorar**, na seção **Queries**, observe que esta consulta é salva em **My queries** como **SQL query 2**. Isso fornece uma opção para renomear a consulta e salvá-la para uso futuro. Também há uma opção para visualizar consultas compartilhadas com você usando a pasta **Shared queries**.
