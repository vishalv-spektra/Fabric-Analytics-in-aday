...
3. レイクハウス、セマンティック モデル、**SQL エンドポイント**という 3 つの要素が lh_FAIAD に関連付けられていることがわかります。前のラボでは、レイクハウスについて説明し、SQL 分析エンドポイントを使用してビジュアル クエリと SQL クエリを作成しました。左のナビゲーションで **FAIAD_<inject key="Deployment ID" enableCopy="false"/>** を選択し、**lh_FAIAD SQL 分析エンドポイント** オプションを選択してこのオプションの探索を続けます。エクスプローラーの **SQL ビュ**ーが表示されます。
